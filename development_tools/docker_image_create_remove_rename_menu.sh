...
docker__menu__sub() {
    ...
    do
        ...
        duplicate_char__func "${DOCKER__DASH}" "${DOCKER__TABLEWIDTH}"
        echo -e "${DOCKER__FOURSPACES}1. Create ${DOCKER__FG_BORDEAUX}image${DOCKER__NOCOLOR} from ${DOCKER__FG_PURPLE}Repository${DOCKER__NOCOLOR}"
        echo -e "${DOCKER__FOURSPACES}2. Create ${DOCKER__FG_BORDEAUX}image${DOCKER__NOCOLOR} from ${DOCKER__FG_BRIGHTPRUPLE}Container${DOCKER__NOCOLOR}"
        echo -e "${DOCKER__FOURSPACES}3. Remove ${DOCKER__FG_BORDEAUX}image${DOCKER__NOCOLOR}"
        echo -e "${DOCKER__FOURSPACES}4. Rename ${DOCKER__FG_BORDEAUX}image${DOCKER__NOCOLOR} ${DOCKER__FG_PURPLE}Repository${DOCKER__NOCOLOR}:${DOCKER__FG_PINK}Tag${DOCKER__NOCOLOR}"
        duplicate_char__func "${DOCKER__DASH}" "${DOCKER__TABLEWIDTH}"
