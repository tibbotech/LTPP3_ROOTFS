...
docker__load_handler__sub() {
    ...
    local images_list_fpath_string=`find ${docker__docker_images_dir} -maxdepth 1 -type f`
    local arr_line=""

    #Check if '' is an EMPTY STRING
    if [[ -z ${images_list_fpath_string} ]]; then
        echo -e "${DOCKER__FIVE_SPACES}No image found at location: ${DOCKER__FILES_FG_ORANGE}${docker__docker_images_dir}${DOCKER__NOCOLOR}"
    else
    ...
    do
        ...
        do
            ...
            if [[ ${mychoice} =~ [1-9,0,m] ]]; then
                ...
                if [[ ${mychoice} -lt ${seqnum} ]] && [[ ${mychoice} -ne 0 ]]; then
                    docker__image_fpath=${docker__docker_images_dir}/${images_list_filename}

                    break   #exit loop

                elif [[ ${mychoice} == "m" ]]; then
                    echo -e "\r"
                    echo -e "Provide ${DOCKER__FILES_FG_ORANGE}Image-file full-path${DOCKER__NOCOLOR} (e.g. /my-other-location/image.tar.gz)?"
                    read -e -p "Input: " docker__image_fpath
        ...
        if [[ -f ${docker__image_fpath} ]]; then
            echo -e "\r"
            echo -e "Loading image: ${DOCKER__FILES_FG_ORANGE}${docker__image_fpath}${DOCKER__NOCOLOR}"
            echo -e "\r"
            ...
            echo -e "\r"
            echo -e "Image-file full-path: ${DOCKER__ERROR_FG_LIGHTRED}${docker__image_fpath}${DOCKER__NOCOLOR} does NOT exist"
