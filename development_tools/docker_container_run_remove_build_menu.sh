...
docker__load_constants__sub() {
    ...
    DOCKER__MENUTITLE+="${DOCKER__FG_DARKBLUE}BUILD ${DOCKER__FG_LIGHTBLUE}CONTAINER${DOCKER__NOCOLOR}"
}
...
docker__menu__sub() {
    ...
    do
        ...
        duplicate_char__func "${DOCKER__DASH}" "${DOCKER__TABLEWIDTH}"
        echo -e "${DOCKER__FOURSPACES}b. build ${DOCKER__FG_LIGHTGREY}ISPBOOOT.BIN${DOCKER__NOCOLOR}"
        duplicate_char__func "${DOCKER__DASH}" "${DOCKER__TABLEWIDTH}"
